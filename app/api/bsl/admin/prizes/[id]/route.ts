...
export async function PATCH(
  ...
  try {
    ...
    const [updated] = await db
      .update(bslPrizes)
      .set(patch)
      .where(eq(bslPrizes.id, id))
      .returning();
    if (!updated)
      return Response.json({ message: "Prize not found" }, { status: 404 });
    await audit(user, "ADMIN_UPDATE_PRIZE", "bsl_prizes", id, patch);
...
export async function DELETE(
  ...
  try {
    ...
    await audit(user, "ADMIN_DELETE_PRIZE", "bsl_prizes", id, null);
